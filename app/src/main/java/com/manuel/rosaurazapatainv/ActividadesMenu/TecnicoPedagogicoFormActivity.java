...
public class TecnicoPedagogicoFormActivity extends AppCompatActivity implements Estado, AdapterView.OnItemSelectedListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_tecnico_pedagogico_form);
        ...
        materialTextView = findViewById(R.id.materialTextEstadoTP);
        materialCardView_agregar = findViewById(R.id.agregarTecnicoPedagogico);
        materialCardView_limpiar = findViewById(R.id.limpiarTecnicoPedagogico);
        textViewAgregarT = findViewById(R.id.textViewAgregarT);
        imageViewEdit = findViewById(R.id.imageViewCheckT);
        titulodelmenu = findViewById(R.id.tituloDelMenu);
        spinner = findViewById(R.id.spinnerEstadoTP);
        ...
        materialCardView_agregar.setOnClickListener(view -> {
            ...
            if (evaluarDatos(numero, textInputLayout_numero) && evaluarDatos(descripcion, textInputLayout_descripcion) && evaluarDatos(cantidad, textInputLayout_cantidad)) {
                ...
                if (getIntent().getBooleanExtra("dato", false)) {
                    new FirebaseHelper().editar(TecnicoPedagogicoFormActivity.this, Colecciones.TECNICOPEDAGOGICO.toString(), pedagogico.getId(), StaticHelper.TECNICOPEDAGOGICOKEYS, datos, TecnicoPedagogicoFormActivity.this);
                } else {
                    new FirebaseHelper().add(TecnicoPedagogicoFormActivity.this, Colecciones.TECNICOPEDAGOGICO.toString(), StaticHelper.TECNICOPEDAGOGICOKEYS, datos, TecnicoPedagogicoFormActivity.this);
                }
                Intent intent = new Intent(getApplicationContext(), TecnicoPedagogicoListaActivity.class);
    ...
    public void getDatos() {
        if (getIntent().getBooleanExtra("dato", false)) {
            pedagogico = (TecnicoPedagogico) getIntent().getSerializableExtra("Técnico Pedagógico");
            Objects.requireNonNull(textInputLayout_numero.getEditText()).setText(pedagogico.getNumero());
            Objects.requireNonNull(textInputLayout_descripcion.getEditText()).setText(pedagogico.getDescripcion());
            Objects.requireNonNull(textInputLayout_cantidad.getEditText()).setText(pedagogico.getCantidad());
            switch (pedagogico.getEstado()) {
                case "Bueno":
                ...
                case "Malo":
                    ...
                    break;
                default:
                    throw new IllegalStateException("Unexpected value: " + pedagogico.getEstado());
            }
            materialTextView.setText("Estado: " + pedagogico.getEstado());
    ...
    public void estado(String mensaje) {
        Toast.makeText(TecnicoPedagogicoFormActivity.this, mensaje, Toast.LENGTH_SHORT).show();
    ...
    public void onBackPressed() {
        super.onBackPressed();
        Intent intent = new Intent(this, TecnicoPedagogicoListaActivity.class);
        startActivity(intent);
        finish();
    }
