...
public class MyItemTouchHelperCallbackA extends ItemTouchHelper.Callback {
    CallBackItemTouch callBackItemTouch;

    public MyItemTouchHelperCallbackA(CallBackItemTouch callBackItemTouch) {
    ...
    public void onChildDraw(@NonNull @NotNull Canvas c, @NonNull @NotNull RecyclerView recyclerView, @NonNull @NotNull RecyclerView.ViewHolder viewHolder, float dX, float dY, int actionState, boolean isCurrentlyActive) {
        if (actionState == ItemTouchHelper.ACTION_STATE_DRAG) {
            ...
            final View foreGroundView = ((AdapterItemActivoFijo.ViewHolderActivoFijo) viewHolder).viewB1;
    ...
    public void onChildDrawOver(@NonNull @NotNull Canvas c, @NonNull @NotNull RecyclerView recyclerView, RecyclerView.ViewHolder viewHolder, float dX, float dY, int actionState, boolean isCurrentlyActive) {
        if (actionState != ItemTouchHelper.ACTION_STATE_DRAG) {
            final View foreGroundView = ((AdapterItemActivoFijo.ViewHolderActivoFijo) viewHolder).viewF1;
    ...
    public void clearView(@NonNull @NotNull RecyclerView recyclerView, @NonNull @NotNull RecyclerView.ViewHolder viewHolder) {
        final View foreGroundView = ((AdapterItemActivoFijo.ViewHolderActivoFijo) viewHolder).viewF1;
        foreGroundView.setBackgroundColor(ContextCompat.getColor(((AdapterItemActivoFijo.ViewHolderActivoFijo) viewHolder).viewF1.getContext(), R.color.white));
